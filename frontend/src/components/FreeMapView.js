...
const MapControls = ({ onFilterChange, selectedFilter, eventCounts, availableTypes }) => {
  ...
  return (
    <div className="absolute top-4 left-4" style={{ zIndex: 1 }}>
      ...
      {showDropdown && (
        <div 
          className="mt-2 bg-white/95 rounded-xl shadow-xl border border-gray-200 p-2 min-w-[200px] max-h-80 overflow-y-auto"
          style={{ zIndex: 2 }}
        >
          {/* All Events */}
          <button
...
const FreeMapView = ({ events, selectedEvent, onEventSelect, onEventDeselect, onShowChat, userLocation, onLocationRefresh }) => {
  ...
  return (
    <div style={{ height: 'calc(100vh - 4rem)', width: '100%', position: 'relative' }} data-testid="map-container">
      ...
      <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 p-3" style={{ zIndex: 1 }}>
      ...
      <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 px-3 py-2" style={{ zIndex: 1 }}>
